...
#SRC=$1
#DST=$2
OUTPUT="$TMP/weaved_plugin"

TEST_GUID="74F3A4D5-F1A1-02BA-989C-790801FE7AC8"
...
usage()
{
        if [ $LOGGING -gt 0 ]; then; logger "[task_notify.sh Called with bad format or -h]"; fi
...
if [ $LOGGING -gt 0 ]; then; logger "[task_notify.sh Called with cmd $cmd taskid $task_id value $@]"; fi
...
case $cmd in
    "0")
        ...
        #resp=$($CURL $CURL_OPS -w "%{http_code}\\n" -X POST -H "Content-Type: application/json" -o "$OUTPUT" $URL -d $data)
